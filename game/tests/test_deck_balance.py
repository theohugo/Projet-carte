...
class BalancedDeckTests(TestCase):
    ...
    def test_real_catalogue_keeps_every_type_between_eight_and_ten_cards(self):
    ...
    def test_dual_type_card_is_counted_once_in_each_type(self):
        dual_type = self.cards["zapdos"]
        dual_type.secondary_type = self.cards["bulbasaur"].primary_type
        dual_type.save(update_fields=["secondary_type"])

        counts = count_cards_per_type([dual_type])

        self.assertEqual(counts["flying"], 1)
        self.assertEqual(counts["grass"], 1)
